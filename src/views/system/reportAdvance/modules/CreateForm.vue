<template>
  <a-drawer width="35%" :label-col="4" :wrapper-col="14" :visible="open" @close="onClose">
    ...
    <a-form-model ref="form" :model="form" :rules="rules">
      <a-form-model-item label="预付金额" prop="amount">
        <a-input v-model="form.amount" placeholder="请输入预付金额"/>
      </a-form-model-item>
      ...
      <a-form-model-item label="单位id" prop="advanceColId">
        <a-input v-model="form.advanceColId" placeholder="请输入单位id"/>
      </a-form-model-item>
...
<script>
import {getReportAdvance, addReportAdvance, updateReportAdvance} from '@/api/system/reportAdvance'
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row, ids) {
      ...
      getReportAdvance(amount).then(response => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          if (this.form.amount !== undefined && this.form.amount !== null) {
            updateReportAdvance(this.form).then(response => {
            ...
            addReportAdvance(this.form).then(response => {
              this.$message.success(
                '新增成功',
                3
              )
